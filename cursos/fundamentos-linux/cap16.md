# Capítulo 16: Permisos Especiales y el Estándar de Jerarquía del Sistema de Archivos
...
> **Es bueno ser buscado.** El 75% de los profesionales de Linux han recibido una llamada de un reclutador en los últimos 6 meses. Casi la mitad recibieron más de 6 llamadas.

## 16.2 El Permiso setuid

Cuando el permiso **setuid** se encuentra en un archivo binario ejecutable (alias, un programa), el archivo binario se «ejecuta como» el propietario del archivo, no como el usuario que lo ejecuta. Este permiso se establece en una cierta cantidad de utilidades del sistema para que puedan ser manejadas por los usuarios normales, pero ejecutadas con los permisos root, proporcionando acceso a los archivos del sistema a los que el usuario normal normalmente no tiene acceso.
...
Como puedes ver, el archivo `/etc/shadow` no se puede ver (o modificar) por los usuarios normales debido a que los permisos del archivo `/etc/shadow` son: `-rw-r-----`. Dado que el archivo es propiedad del usuario root, el administrador del sistema podría modificar temporalmente los permisos en el caso de que quisiera ver o modificar este archivo.
...
Al igual que los permisos de lectura, escritura y ejecución, los permisos especiales pueden ser ajustados con el comando `chmod`, utilizando cualquiera de los métodos, simbólicos y octales.
...
En el capítulo anterior, establecimos permisos con el método octal utilizando códigos de tres dígitos. Cuando se proporciona un código de tres dígitos, el comando `chmod` supone que el primer dígito antes del código de tres dígitos es `0`. Los permisos especiales serán establecidos sólo si se fijan los cuatro dígitos. Si se especifican tres dígitos cuando se cambian los permisos en un archivo que ya tiene un conjunto de permisos especiales, el primer dígito se establecerá en `0` y el permiso especial se quita del archivo.
...
- `bob` es miembro del grupo `payroll`
- `sue` es miembro del grupo `staff`
- `tim` es miembro del grupo `acct`
...
1. Crea un nuevo grupo llamado `team`.
2. Añade `bob`, `sue` y `tim` al grupo `team`.
3. Hace un nuevo directorio llamado `/home/team`.
4. Hace que el grupo propietario del directorio `/home/team` sea el grupo `team`.
...
Para agregar el permiso setgid numéricamente, añade 2000 a los permisos existentes del archivo (asume en el ejemplo siguiente que el directorio tenía originalmente 775 para su permiso):
...
Una `s` minúscula significa que tanto el permiso setgid como el permiso de ejecución de grupo están establecidos. Una `S` mayúscula significa que sólo el permiso setgid está establecido y no el permiso de ejecución del grupo: `drwxrwSr-x`.
...
Ésta es una situación en la que los **enlaces físicos** y los **enlaces simbólicos** (blandos) pueden ser útiles. Se puede crear un archivo que se enlace al que está «profundamente enterrado». Este nuevo archivo podría ser colocado en tu directorio home o cualquier otro lugar conveniente. Cuando accedes al archivo nuevo de «enlace», éste accederá a los contenidos del archivo `valuable-information.txt`.

Cada método de enlace, físico o simbólico, resulta en el mismo acceso global, pero utilizan diferentes técnicas. Existen ventajas y desventajas de cada método, por lo que conocer ambas técnicas y cuándo se usan es importante.

### 16.7.1 Creación de los Vínculos Físicos

Con el fin de entender los enlaces físicos, hay que entender un poco cómo el sistema de archivos realiza un seguimiento de los archivos. Por cada archivo creado, hay un bloque de datos en el sistema de archivos que almacena la **información meta** del archivo. La información meta incluye información sobre el archivo, tal como los permisos, propiedades y marcas de tiempo. La información meta no incluye el nombre del archivo o el contenido del archivo, pero sí incluye casi toda la demás información sobre el archivo.
...
Debido a que tanto el archivo `passwd` como `mypasswd` tienen el mismo número de inodo, en realidad, son el mismo archivo. Puedes acceder a los datos del archivo utilizando cualquiera de los dos nombres de archivo.
...
<figure>
<img src="diagrams/cap16-enlaces-fisicos-simbolicos.svg" alt="Comparación entre enlaces físicos, donde dos nombres de archivo apuntan al mismo inodo, y enlaces simbólicos, donde un archivo apunta a una ruta que a su vez lleva al inodo">
<figcaption>Enlaces físicos (mismo inodo) frente a enlaces simbólicos (apuntan a una ruta).</figcaption>
</figure>
...
|---|---|
| `/` | La base de la estructura, o el root del sistema de archivos; este directorio unifica todos los directorios independientemente de si son particiones locales, dispositivos extraíbles o recursos compartidos de red |
...
| `/lib<qual>` | Las librerías esenciales creadas para una arquitectura específica. Por ejemplo, el directorio `/lib64` para los procesadores de 64 bits AMD/Intel x86 compatibles |
...
| `/usr/local/src` | La ubicación en la que a menudo se coloca el código fuente de software para ser compilado localmente |

<figure>
<img src="diagrams/cap16-fhs-arbol.svg" alt="Árbol del Estándar de Jerarquía del Sistema de Archivos: la raíz / contiene directorios esenciales como bin, boot, dev, etc, home, lib, sbin y tmp, y jerarquías expandidas /usr (con bin, lib, local, share) y /var (con log, spool, tmp)">
<figcaption>Árbol simplificado del Estándar de Jerarquía del Sistema de Archivos (FHS).</figcaption>
</figure>
...
Puede haber varias versiones de una librería debido a que el código puede ser diferente dentro de cada archivo a pesar de que puede llevar a cabo funciones similares a las otras versiones de la librería. Una de las razones por las que el código puede ser diferente, a pesar de que puede hacer lo mismo que otro archivo de la biblioteca, es que está compilado para ejecutarse en un tipo diferente de procesador. Por ejemplo, es habitual que los sistemas que utilizan un código diseñado para los procesadores de tipo Intel/AMD de 64 bits, tengan las dos bibliotecas, la de 32 bits y de 64 bits.
...
- Para dar soporte a los ejecutables `/usr/bin` y `/usr/sbin`, normalmente se usan los directorios librería `/usr/lib` y `/usr/lib64`.
...
- El permiso especial `setuid` (bit `s` en la posición de ejecución del usuario, ej. `-rwsr-xr-x`) hace que un binario ejecutable se ejecute con los privilegios de su propietario, no del usuario que lo invoca; es la razón por la que comandos como `passwd` pueden modificar `/etc/shadow`.
- El permiso `setgid` en un archivo ejecutable otorga temporalmente los privilegios del grupo propietario del archivo (ej. `/usr/bin/wall` con el grupo `tty`); en un directorio, hace que los archivos y subdirectorios nuevos hereden automáticamente el grupo propietario del directorio, en lugar del grupo primario del usuario que los crea.
- El **sticky bit** en un directorio compartido (ej. `/tmp`) permite que cualquier usuario cree archivos, pero restringe el borrado de cada archivo a su propietario o al usuario root.
- Los permisos especiales se configuran con `chmod` simbólico (`u+s`, `g+s`, `o+t`) o numérico añadiendo/restando 4000 (setuid), 2000 (setgid) o 1000 (sticky bit) al modo de tres dígitos; una `S` o `T` mayúscula en el listado indica que el permiso especial está fijado pero el permiso de ejecución correspondiente falta.
- Los **enlaces físicos** son nombres de archivo adicionales que apuntan al mismo número de inodo (no pueden cruzar sistemas de archivos ni enlazar directorios), mientras que los **enlaces simbólicos** son archivos independientes que apuntan a una ruta (más flexibles, pero con un único punto de fallo si se borra el archivo original).
- El **Estándar de Jerarquía del Sistema de Archivos (FHS)** clasifica los directorios de Linux según si son compartibles y si su contenido es estático o variable, y define la ubicación estándar de binarios (`/bin`, `/usr/bin`, `/usr/local/bin`), configuración (`/etc`), librerías (`/lib`, `/usr/lib`), datos variables (`/var/log`, `/var/spool`) y directorios home (`/home`).
